...
using HastaneProje.Models.UyeIslemleri;
...
namespace HastaneProje.Controllers
{
    public class UyeIslemleriController : Controller
    {
        ...
        // Class tepesine koyarsak bunu tüm mehodlarda kullanıcı girişi ister.
        [HttpPost]
        public async Task<IActionResult> UyeOlBashekim(RegisterViewModel registerViewModel)
        {
            ...
            if (sonuc.Succeeded)//Eğer yapılan işlem başarılı olduysa yani kullanıcı bilgisi veritabanına kayıt edildiyse.
            {
                ...
                var DonusUrl = Url.Action("MailOnayla", "UyeIslemleri", new { userId = kullanici.Id, code = kodBilgisi.Result }); //Dönüş URL Tanımlıyoruz ilgili method, controler ve nesne istediği için ilgili nesneyi gönderdik. 

                // /UyeIslemleri/MailOnayla?userId=12312.123123.1241.51251.2123&code=123128db127812badg8712gds8dg18dbgs871g


                #region kullanıcı yetkilendirme işlemleri
                /* KULLANICI ROLE YETKİLENDİRME İŞLEMLERİ */
                await _userManager.AddToRoleAsync(kullanici, "Başhekim");
                /* KULLANICI ROLE YETKİLENDİRME İŞLEMLERİ */
                #endregion
        ...
        // Class tepesine koyarsak bunu tüm mehodlarda kullanıcı girişi ister.
        [HttpPost]
        public async Task<IActionResult> DoktorEkle(RegisterViewModel registerViewModel)
        {
            if (registerViewModel.PasswordTekrar != registerViewModel.Password)
            {
                ModelState.AddModelError(string.Empty, "Girmiş olduğunuz şifreler birbirleri ile uyuşmuyor...");
        ...
        // Class tepesine koyarsak bunu tüm mehodlarda kullanıcı girişi ister.
        [HttpPost]
        public async Task<IActionResult> SekreterEkle(RegisterViewModel registerViewModel)
        {
            if (registerViewModel.PasswordTekrar != registerViewModel.Password)
            {
                ModelState.AddModelError(string.Empty, "Girmiş olduğunuz şifreler birbirleri ile uyuşmuyor...");
            ...
            if (sonuc.Succeeded)//Eğer yapılan işlem başarılı olduysa yani kullanıcı bilgisi veritabanına kayıt edildiyse.
            {
                ...
                #region kullanıcı yetkilendirme işlemleri
                /* KULLANICI ROLE YETKİLENDİRME İŞLEMLERİ */
                await _userManager.AddToRoleAsync(kullanici, "Sekreter");
                /* KULLANICI ROLE YETKİLENDİRME İŞLEMLERİ */
                #endregion
        ...
        // Class tepesine koyarsak bunu tüm mehodlarda kullanıcı girişi ister.
        [HttpPost]
        public async Task<IActionResult> HastaKayit(RegisterViewModel registerViewModel)
        {
            if (registerViewModel.PasswordTekrar != registerViewModel.Password)
            {
                ModelState.AddModelError(string.Empty, "Girmiş olduğunuz şifreler birbirleri ile uyuşmuyor...");
        ...
        public async Task<IActionResult> SifremiUnuttum(string mail)
        {
            ...
            var DonusUrl = Url.Action("SifreSifirlamaYap", "UyeIslemleri", new { userId = kullanici.Id, code = DogrulamaKodu }); //Dönüş URL Tanımlıyoruz ilgili method, controler ve nesne istediği için ilgili nesneyi gönderdik. 
